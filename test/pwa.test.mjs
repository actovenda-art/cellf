...
test('as rotas de produção entregam manifesto e imagens antes do fallback HTML', () => {
  const { routes } = JSON.parse(readFileSync(new URL('../vercel.json', import.meta.url), 'utf8'));
  for (const path of ['/manifest.webmanifest', '/cellf-distorted-glass.svg', '/favicon-32.png', '/apple-touch-icon.png', ...manifest.icons.map(icon => icon.src)]) {
    ...
    if (path.endsWith('.png')) {
      ...
      assert.equal(route.headers['Content-Type'], 'image/svg+xml');
      assert.match(bytes.toString('utf8'), /^<svg\b/u);
    } else {
